...
public class QueryExecutors {
                    ...
                    .createQuery(sqlClient, type, ExecutionPurpose.QUERY, FilterLevel.DEFAULT, (q, table) -> {
                        ...
                        if (specification != null) {
                            if (specification instanceof KSpecification<?>) {
                                JSpecification<?, Table<?>> spec = (JSpecification<?, Table<?>>) (JSpecification<?, ?>) KSpecificationKt
                                        .toJavaSpecification(((KSpecification<Object>) specification));
                                q.where(spec);
                            } else {
                        ...
                        if (sort != null) {
                            q.orderBy(QuarkusOrders.toOrders(table, sort));
                        }
                        if (fetcher != null) {
            ...
            switch (queryData.getAction()) {
                case FIND:
                    if (returnType == org.babyfish.jimmer.Page.class) {
                        return query.fetchPage(pagination.index, pagination.size);
                    }
    ...
    private static org.babyfish.jimmer.sql.ast.Predicate astPredicate(
            Table<?> table,
            io.quarkiverse.jimmer.runtime.repository.parser.Predicate predicate,
            Object[] args) {
        if (predicate == null) {
            return null;
        }
        ...
        if (predicate instanceof AndPredicate) {
            List<io.quarkiverse.jimmer.runtime.repository.parser.Predicate> subPredicates = ((AndPredicate) predicate)
                    .getPredicates();
            Predicate[] subAstPredicates = new Predicate[subPredicates.size()];
            int index = 0;
            for (io.quarkiverse.jimmer.runtime.repository.parser.Predicate subPredicate : subPredicates) {
                subAstPredicates[index++] = astPredicate(table, subPredicate, args);
            }
            return Predicate.and(subAstPredicates);
        }
        if (predicate instanceof OrPredicate) {
            List<io.quarkiverse.jimmer.runtime.repository.parser.Predicate> subPredicates = ((OrPredicate) predicate)
                    .getPredicates();
            Predicate[] subAstPredicates = new Predicate[subPredicates.size()];
            int index = 0;
            for (io.quarkiverse.jimmer.runtime.repository.parser.Predicate subPredicate : subPredicates) {
