...
public class QuarkusTransactionCacheOperator extends AbstractCacheOperator {
    ...
    private static final String SELECT_ID_PREFIX = "select " +
            ID +
            " from " +
            TABLE_NAME +
            " order by " +
    ...
    public QuarkusTransactionCacheOperator() {
        this(null, 32);
    }

    public QuarkusTransactionCacheOperator(int batchSize) {
        this(null, batchSize);
    }

    public QuarkusTransactionCacheOperator(ObjectMapper mapper) {
        this(mapper, 32);
    }

    public QuarkusTransactionCacheOperator(ObjectMapper mapper, int batchSize) {
        if (batchSize < 1) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    Object key = mapper.readValue(
                            json,
                            type != null ? (Class<Object>) type.getIdProp().getElementClass()
    ...
    private static class MergedKey {
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equals(type, mergedKey.type) && Objects.equals(prop, mergedKey.prop)
                    && Objects.equals(reason, mergedKey.reason);
        }
